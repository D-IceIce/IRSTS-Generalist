...
class Attention(torch.nn.Module):
    ...
    def forward(self, x, global_attn=False):  # x (B,N,C)
        B, N, _ = x.shape
...
class TinyViTBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        ...
        if H == self.window_size and W == self.window_size:
            ...
            if return_attention:
                _, attn = self.attn(x, global_attn=True)
                return attn
            ...
            # window partition
            x = x.view(B, nH, self.window_size, nW, self.window_size, C) # 1*19*7*19*7*128
            x = x.transpose(2, 3) # 1*19*19*7*7*128
            x = x.reshape(B * nH * nW, self.window_size * self.window_size, C) # 361*49*128
            x, attn = self.attn(x)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        # Iterate through all blocks, returning attention from the last block if requested
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x)
            else:
                if return_attention and blk == self.blocks[-1]:
                    # If it's the last block, capture the attention
...
class TinyViT(nn.Module):
    ...
    def get_last_selfattention(self, x):
        """Extract attention from the last TinyViT block"""
        ...
        attention = last_layer(x, return_attention=True)

        if x.shape[0] > 1:
            attention = attention.mean(dim=0)

        return attention  # Return the last self-attention
